...
var import_firestore_types = require("@firebase/firestore-types");
...
var timestampSchema = import_zod.z.custom(
  (val) => val instanceof import_firestore_types.Timestamp
);
...
var toFirestore = {
  date: (date) => import_firestore_types.Timestamp.fromDate(date),
  ...
  for (const key in sentMessages) {
    ...
    if (message) {
      const firestoreMessage = {
        ...message,
        created_at: message.created_at instanceof Date ? import_firestore.Timestamp.fromDate(message.created_at) : message.created_at,
        updated_at: message.updated_at instanceof Date ? import_firestore.Timestamp.fromDate(message.updated_at) : message.updated_at
      };
...
var convertSentMessagesFromFirestore = (firestoreSentMessages) => {
  ...
  for (const key in firestoreSentMessages) {
    ...
    if (firestoreMessage) {
      const appMessage = {
        ...firestoreMessage,
        created_at: firestoreMessage.created_at instanceof import_firestore.Timestamp ? firestoreMessage.created_at.toDate() : firestoreMessage.created_at,
        updated_at: firestoreMessage.updated_at instanceof import_firestore.Timestamp ? firestoreMessage.updated_at.toDate() : firestoreMessage.updated_at
      };
...
var import_firestore_types2 = require("@firebase/firestore-types");
...
var userFromFirestore = (firestoreUser) => {
  return genericFromFirestore({
    ...
    dateFieldMappings: dateFieldMappings8,
    specialCaseHandler: (result, firestoreData) => {
      if (firestoreData.balance instanceof import_firestore_types2.FieldValue) {
        result.balance = null;
      }
    }
  });
