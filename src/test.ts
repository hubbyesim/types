import { exampleUsage, exampleInvalidUser } from './examples/zodExample';
import { z } from 'zod';
import {
    packageAppSchema,
    packageToFirestore,
...
try {
    ...
    const firestoreCountry = countryToFirestore(validCountry);
    ...
    console.log('Country round trip successful:',
        countryData.id === retrievedCountry.id &&
...
try {
    ...
    // Convert to Firestore
    const firestoreCurrency = currencyToFirestore(validCurrency);
    ...
    // Check data integrity
    console.log('Currency round trip successful:',
        currencyData.id === retrievedCurrency.id &&
...
try {
    ...
    const validApiLog = apiLogAppSchema.parse(apiLogData);
    console.log('ApiLog validation successful');

    // Convert to Firestore
    const firestoreApiLog = apiLogToFirestore(validApiLog);
    console.log('ApiLog to Firestore conversion successful');

    // Convert back to app
    const retrievedApiLog = apiLogFromFirestore(firestoreApiLog);
    console.log('ApiLog to App conversion successful');

    // Check data integrity
    console.log('ApiLog round trip successful:',
        apiLogData.id === retrievedApiLog.id &&
...
try {
    ...
    const validPackageSpec = packageSpecificationSchema.parse(packageSpec);
    ...
    const validBookingApiRequest = bookingApiRequestSchema.parse(bookingApiRequest);
